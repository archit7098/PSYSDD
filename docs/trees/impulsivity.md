...
```mermaid
flowchart TD
  START(["Impulsivity or impulse-control problems"]) --> Nn1
  Nn1{"Is the impulsivity due to a substance/medication or general medical condition?"}
  Nn1 -->|Yes| T0["Substance/medical etiology → Substance-Induced or Medical cause"]
  Nn1 -->|No| Nn2
  Nn2{"Does it occur only during manic/hypomanic episodes?"}
  Nn2 -->|Yes| T1["A bipolar disorder → Bipolar I/II Disorder"]
  Nn2 -->|No| Nn3
  Nn3{"Is there a persistent, cross-situational pattern of impulsivity with inattention/hyperacti"}
  Nn3 -->|Yes| T2["Attention-Deficit/Hyperactivity Disorder → Attention-Deficit/Hyperactivity Disorder"]
  Nn3 -->|No| Nn4
  Nn4{"Is the specific behavior recurrent stealing of unneeded objects?"}
  Nn4 -->|Yes| T3["Kleptomania → Kleptomania"]
  Nn4 -->|No| Nn5
  Nn5{"Is it deliberate fire setting with arousal/fascination?"}
  Nn5 -->|Yes| T4["Pyromania → Pyromania"]
  Nn5 -->|No| Nn6
  Nn6{"Is it persistent, recurrent problematic gambling?"}
  Nn6 -->|Yes| T5["Gambling Disorder → Gambling Disorder"]
  Nn6 -->|No| Nn7
  Nn7{"Is it recurrent hair pulling or skin picking with attempts to stop?"}
  Nn7 -->|Yes| T6["Trichotillomania or Excoriation Disorder → Trichotillomania; Excoriation (Skin-Picking) Di"]
  Nn7 -->|No| Nn8
  Nn8{"Is impulsivity part of a pervasive personality pattern (borderline, antisocial)?"}
  Nn8 -->|Yes| T7["A personality disorder → Borderline Personality Disorder; Antisocial Personality Disorder"]
  Nn8 -->|No| T8["Impulsivity not attributable to a specific disorder — reassess context"]
```
